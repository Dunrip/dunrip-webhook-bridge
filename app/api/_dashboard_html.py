  ...
  <main x-show="authenticated" class="max-w-7xl mx-auto px-4 py-6 space-y-6">
    ...
    <section>
      ...
      <div class="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
        <div class="overflow-x-auto">
          <table class="w-full text-sm">
            ...
            <tbody>
              ...
              <template x-for="d in deliveries" :key="rowKey(d)">
                <tr class="border-b border-gray-800/50 hover:bg-gray-800/50 transition cursor-pointer"
                    @click="toggleRow(rowKey(d))">
                  <td class="px-4 py-3 font-mono text-xs text-gray-400" x-text="(d.inbound_delivery_id || d.delivery_id || d.id || '').slice(0, 12)"></td>
                  <td class="px-4 py-3" x-text="d.provider || d.source || 'github'"></td>
                  <td class="px-4 py-3" x-text="d.reason || d.event_type || d.event || '-'"></td>
                  <td class="px-4 py-3">
                    <span class="px-2 py-0.5 rounded-full text-xs font-medium"
                          :class="statusColor(d.status)" x-text="d.status"></span>
                  </td>
                  <td class="px-4 py-3 text-gray-400 text-xs" x-text="timeAgo(deliveryTs(d))"></td>
                  <td class="px-4 py-3 text-right">
                    <button @click.stop="replayDelivery(d.delivery_id || d.id)"
                            :disabled="!(d.delivery_id || d.id)"
                            class="bg-indigo-600 hover:bg-indigo-500 disabled:opacity-40 disabled:cursor-not-allowed text-white text-xs rounded px-2 py-1 transition">
                      Replay
                    </button>
                  </td>
                </tr>
        ...
        <template x-for="d in deliveries" :key="'detail-' + rowKey(d)">
          <div x-show="expandedRows[rowKey(d)]"
               x-transition
      ...
      return {
        ...
        async fetchDeliveries() {
          ...
          try {
            ...
            const res = await fetch(`/deliveries/recent?${params}`, {
              headers: { 'X-Api-Key': this.apiKey }
            });
            if (res.ok) {
        ...
        startPollingFallback() {
          ...
          this._pollInterval = setInterval(async () => {
            ...
            const newEntries = this.deliveries
              .filter(d => {
                const id = this.rowKey(d);
                return id && !seenIds.has(id);
              })
              .map(d => ({
                _id: this.rowKey(d),
                time: new Date().toLocaleTimeString('en-US', { hour12: false }),
                event_type: d.reason || d.event_type || d.event || 'delivery',
                status: d.status || '',
                summary: `id=${this.rowKey(d).slice(0, 12)} (polled)`,
              }));
        ...
        rowKey(d) {
          return d.inbound_delivery_id || d.delivery_id || d.id || JSON.stringify(d).slice(0, 64);
        },

        deliveryTs(d) {
          const transitions = d.status_transitions || [];
          if (transitions.length && transitions[transitions.length - 1].at) return transitions[transitions.length - 1].at;
          return d.first_seen || d.timestamp || d.created_at;
        },
